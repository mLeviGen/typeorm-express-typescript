...
export const validatorCreateCustomer = (req: Request, _res: Response, next: NextFunction) => {
  const { name, phone, addressId, address } = req.body ?? {};

  if (!name || validator.isEmpty(String(name))) {
    throw new CustomError(400, 'Validation', 'Customer validation error', ['Customer name is required']);
  }
